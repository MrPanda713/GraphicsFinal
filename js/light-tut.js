...
function addLights(){
	...
	spotLight = new THREE.SpotLight(0xffffff, 1, 20000, 20, 10);
...
function addSceneElements(){
	// Create a cube used to build floor and walls
	var cube = new THREE.CubeGeometry(200,1,200);
	
	// create different materials
	var floorMat = new THREE.MeshPhongMaterial({map:new THREE.TextureLoader().load('textures/wood-floor.jpg')});
	var wallMat = new THREE.MeshPhongMaterial({map:new THREE.TextureLoader().load('textures/bricks.jpg')});
	var redMat = new THREE.MeshPhongMaterial({color: 0xff3300, specular: 0x555555, shininess: 30});
	var purpleMat = new THREE.MeshPhongMaterial({color: 0x6f6cc5, specular: 0x555555, shininesss: 30});
